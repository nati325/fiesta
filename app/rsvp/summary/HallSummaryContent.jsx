...
export default function HallSummaryContent() {
    ...
    useEffect(() => {
        ...
        Promise.all([
            fetch(`/api/events/${encodeURIComponent(eventId)}`).then(r => r.ok ? r.json() : null),
            fetch(`/api/rsvp?eventId=${encodeURIComponent(eventId)}&mode=summary`).then(r => r.ok ? r.json() : [])
        ]).then(([eventInfo, data]) => {
            if (eventInfo?.coupleName) {
                const dateStr = eventInfo.eventDate
                    ? new Date(eventInfo.eventDate).toLocaleDateString('he-IL')
                    : '';
                setEventTitle(`${eventInfo.coupleName}${dateStr ? ` | ${dateStr}` : ''}`);
            }
    ...
    const calculateSummary = (data) => {
        const summary = data.reduce((acc, curr) => {
            ...
            if (!acc.tables[tableNum]) {
                acc.tables[tableNum] = { count: 0, guests: [], vegan: 0, veg: 0 };
            }
    ...
    if (!eventId) {
        return <div className="loading">חסר קוד אירוע בקישור</div>;
    }
        ...
        <div className="hall-view-root" dir="rtl">
            <header className="hall-header">
                <div className="container">
                    <div className="badge">סיכום לאולם</div>
                    <h1>{eventTitle || 'סיכום אירוע'}</h1>
                    <p>דוח ריכוז מנות וסידור שולחנות</p>
                </div>
            </header>
            ...
            <main className="container">
                ...
                <section className="tables-section">
                    ...
                    <div className="tables-grid">
                        {Object.entries(stats.tables).sort((a, b) => {
                            if (a[0] === 'טרם שובץ') return 1;
                            if (b[0] === 'טרם שובץ') return -1;
            ...
            <style jsx>{`
                ...
                .badge { display: inline-block; background: #fbbf24; color: #1e293b; padding: 4px 12px; border-radius: 50px; font-weight: 900; font-size: 0.7rem; margin-bottom: 15px; }
                ...
                .diet-note { color: #ef4444; font-size: 0.8rem; }

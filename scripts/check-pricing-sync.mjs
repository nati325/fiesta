 ...
 *
 * An identical copy of this script lives in fiesta_sapakim/scripts/. When only
 * one repo is checked out — on Vercel, or in a fresh clone — the byte comparison
 * is skipped and only the local copy's arithmetic is exercised.
 */
...
const SIBLING_COPY = path.resolve(APP_ROOT, '..', '..', 'fiesta_sapakim', 'lib', 'pricing.js');
...
if (sibling === null) {
  console.log('· fiesta_sapakim לא נמצא לצד האפליקציה — מדלגים על השוואת העותקים');
